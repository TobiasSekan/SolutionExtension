...
export class solutionHover implements vscode.HoverProvider
{
    ...
    {
        if(this.contains(document, position, "\"{[0-9A-F]{8}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{12}}\""))
        {
            let guid = this.getText(document, position, "\"{[0-9A-F]{8}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{12}}\"");
            guid = guid.replace(/^"(.*)"$/, '$1');

            const projectType = this.getProjectType(guid);
    ...
    private getProjectType(guid: string): string
    {
        switch(guid)
        {
